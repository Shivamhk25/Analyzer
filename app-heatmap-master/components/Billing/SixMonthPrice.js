...
export const discountFun = (basePrice, discountRate) => {
  return (basePrice * (100 - discountRate)) / 100;
};

const SixMonthPrice = ({ currentPlan }) => {
    ...
    let body = {
      data: {
        recurring_application_charge: {
          name: finalPlan,
          price: Math.round((finalPrice * 6 + Number.EPSILON) * 100) / 100,
          // test: true,
              ...
              {isVisible && (
                <>
                  <Text fontSize="15px" color="black" fontWeight="500">
                    ...
                    {Math.round((finalPrice * 6 + Number.EPSILON) * 100) / 100}
                  </Text>
                  <Button
                    // bg="blue"
                    background={"#b983ff"}
