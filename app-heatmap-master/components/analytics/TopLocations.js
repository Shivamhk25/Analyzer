...
import { getTopLocations } from "../../API/analytic";
...
const TopLocations = () => {
  ...
  const fetchTopLocations = async (data) => {
    try {
      const res = await getTopLocations(axiosFetch, data);
      console.log(res, "from fetchTopLocations");
      setTotalItem(res);
    } catch (err) {
      console.log(err);
    }
